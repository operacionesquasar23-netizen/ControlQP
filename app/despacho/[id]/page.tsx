...
function ConfirmarDespachoContenido({ id }: { id: string }) {
  ...
  if (sesion) {
    return <FormularioDespacho id={id} codigoAlmacen={sesion.codigo} nombreAlmacen={sesion.nombre} />;
  }

  return (
    <AccesoAlmacen titulo="Confirmar despacho" descripcion="Ingresa tu código de acceso de almacén.">
      {({ codigoAlmacen, nombreAlmacen }) => (
        <FormularioDespacho id={id} codigoAlmacen={codigoAlmacen} nombreAlmacen={nombreAlmacen} />
      )}
    </AccesoAlmacen>
  );
}
...
function FormularioDespacho({
  ...
  function manejarFoto(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = (ev) => {
      ...
      setFoto({
        ...
        nombre: `despacho_${id}_${Date.now()}.${archivo.type.split('/')[1]}`,
      });
    };
  ...
  async function manejarConfirmar() {
    ...
    let urlFoto = '';
    try {
      urlFoto = await subirFotoDespacho(
        id,
        solped!.cabecera.codigo_campaña,
        foto!.base64,
        foto!.mimeType,
        foto!.nombre
      );
  ...
  if (cargando) {
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center">
        <p className="text-sm text-gray-400">Cargando SOLPED…</p>
      </div>
    );
  }

  if (errorCarga || !solped) {
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center px-4">
        <div className="bg-white rounded-2xl p-8 max-w-md w-full text-center">
          <p className="text-sm text-red-600 mb-4">{errorCarga}</p>
          <Link href={`/despacho?codigo_almacen=${encodeURIComponent(codigoAlmacen)}`}
            className="text-xs text-gray-400 hover:text-blue-700">← Volver a SOLPEDs</Link>
        </div>
      </div>
    );
  }

  if (despachoConfirmado) {
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center px-4">
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-8 max-w-md w-full text-center">
          ...
          <p className="text-sm text-gray-500 mb-6">
            SOLPED <strong>{id}</strong> · Campaña <strong>{solped.cabecera.codigo_campaña}</strong>
          </p>
          <Link
            href={`/despacho?codigo_almacen=${encodeURIComponent(codigoAlmacen)}`}
  ...
  // Agrupar detalle por lugar para mostrar mejor
    ...
    <div className="min-h-screen bg-slate-50">
      <div className="bg-brand text-white px-6 py-5">
        <div className="max-w-3xl mx-auto flex items-center justify-between">
          <Link href={`/despacho?codigo_almacen=${encodeURIComponent(codigoAlmacen)}`}
            className="text-white/80 hover:text-white text-sm">← SOLPEDs</Link>
      ...
      <main className="max-w-3xl mx-auto px-4 py-8">
        ...
        <p className="text-sm text-gray-500 mb-6">
          {solped.cabecera.id_solped} · v{solped.cabecera.version} · {solped.cabecera.cliente} · {solped.cabecera.codigo_campaña}
        </p>
        ...
        <section className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 mb-4">
          ...
          <input
            type="text"
            placeholder="Ej: Entrega parcial, falta un bulto"
            value={observaciones}
