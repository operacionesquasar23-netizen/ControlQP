...
//
// Historial de SOLPEDs de una campaña. Muestra la versión VIGENTE
// destacada arriba, y el historial de versiones anteriores abajo.
// Desde aquí el ejecutivo puede crear una nueva versión (con motivo
// obligatorio) si necesita modificar lo solicitado.
...
function HistorialSolpedContenido({ codigo }: { codigo: string }) {
  ...
  if (sesion) {
    return <HistorialSolped codigo={codigo} codigoEjecutivo={sesion.codigo} nombreEjecutivo={sesion.nombre} />;
  }
...
function HistorialSolped({
  ...
  const historial = solpeds.filter((s) => s.cabecera.estado !== 'vigente');

  function abrirFormNuevaVersion() {
    if (!vigente) return;
    // Pre-cargar las líneas de la versión vigente como punto de partida
    setLineas(
      vigente.detalle.map((d) => ({
  ...
  function totalSolicitadoEnFormulario(nombreProducto: string): number {
    return lineas.reduce((sum, l) => {
      if (l.nombre_producto === nombreProducto) return sum + (Number(l.cantidad_solicitada) || 0);
      return sum;
    }, 0);
  }
  ...
  async function manejarGuardarNuevaVersion() {
    ...
    // Validación de stock (excluyendo la vigente actual que va a ser reemplazada)
    ...
    // Stock disponible ya incluye lo comprometido en la vigente actual,
    // así que lo sumamos de vuelta para no doble-contar
    const stockAjustado: Record<string, number> = { ...stock };
    if (vigente) {
      vigente.detalle.forEach((d) => {
        stockAjustado[d.nombre_producto] = (stockAjustado[d.nombre_producto] || 0) + Number(d.cantidad_solicitada);
      });
    }
  ...
  if (cargando) {
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center">
        <p className="text-sm text-gray-400">Cargando SOLPEDs…</p>
      </div>
    );
  }
      ...
      <div className="bg-brand text-white px-6 py-5">
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <Link href={`/campanas/${codigo}?codigo_ejecutivo=${encodeURIComponent(codigoEjecutivo)}`}
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          {!mostrarFormNuevaVersion && (
            <div className="flex gap-2">
              {vigente ? (
                <button
                  onClick={abrirFormNuevaVersion}
                  className="text-xs font-semibold text-amber-700 hover:text-amber-800 border border-amber-200 rounded-lg px-3 py-2 transition-colors"
                >
                  ✏️ Nueva versión
                </button>
              ) : (
                <Link
                  href={`/solped/nueva?codigo_ejecutivo=${encodeURIComponent(codigoEjecutivo)}&codigo_campaña=${encodeURIComponent(codigo)}`}
          ...
          <section className="bg-white rounded-2xl border border-green-200 shadow-sm p-6 mb-4">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-xs text-gray-400">
                Despacho: {formatearFecha(vigente.cabecera.fecha_despacho)}
              </span>
            </div>
        ...
        {mostrarFormNuevaVersion && (
          <section className="bg-white rounded-2xl border border-amber-200 shadow-sm p-6 mb-4">
            <p className="text-sm font-semibold text-gray-900 mb-4">
              Nueva versión — reemplaza {vigente?.cabecera.id_solped} v{vigente?.cabecera.version}
            </p>

            {erroresForm.length > 0 && (
              <div className="bg-red-50 border border-red-200 rounded-xl px-4 py-3 mb-4 text-red-800">
                <ul className="list-disc pl-5 space-y-0.5">
                  {erroresForm.map((e, i) => <li key={i} className="text-sm">{e}</li>)}
                </ul>
              </div>
            )}
                  ...
                  {Object.keys(stockAjustadoParaVersion(stock, vigente)).map((producto) => {
                    ...
                    return (
                      <span key={producto} className={
                        ...
                        (sol > disp ? 'bg-red-100 text-red-700' : 'bg-white text-gray-700 border border-blue-100')
                      }>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-4">
              ...
              <div>
                ...
                <input
                  type="text"
                  placeholder="Ej: Se agregan 5 unidades extra para tienda X"
                  value={motivoCambio}
            ...
            <button
              type="button"
              onClick={() => setLineas((prev) => [...prev, { nombre_lugar: '', nombre_producto: '', cantidad_solicitada: '' }])}
              className="text-xs font-semibold text-blue-700 border border-blue-200 rounded-lg px-3 py-1.5 mb-4 transition-colors"
            >
              + Agregar línea
            </button>
            ...
            <div className="flex justify-end gap-2">
              <button
                onClick={() => { setMostrarFormNuevaVersion(false); setErroresForm([]); }}
                className="text-xs font-semibold text-gray-600 border border-gray-200 rounded-lg px-4 py-2 hover:bg-gray-50"
              >
                Cancelar
              </button>
              <button
                onClick={manejarGuardarNuevaVersion}
                disabled={guardando}
              ...
              {historial.map((s) => (
                <div key={s.cabecera.id_solped}
                  ...
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-xs text-gray-400">
                      Despacho: {formatearFecha(s.cabecera.fecha_despacho)}
                    </span>
                  </div>
            ...
            <div className="space-y-0.5 pl-3">
              {lineasLugar.map((l, i) => (
                <div key={i} className="flex justify-between">
                  ...
                  <span className={`font-medium text-gray-900 ${compacto ? 'text-xs' : 'text-sm'}`}>
...
function stockAjustadoParaVersion(
  stock: Record<string, number>,
  vigente: SolpedCompleta | undefined
): Record<string, number> {
  const ajustado = { ...stock };
  if (vigente) {
    vigente.detalle.forEach((d) => {
      ajustado[d.nombre_producto] = (ajustado[d.nombre_producto] || 0) + Number(d.cantidad_solicitada);
    });
  }
  return ajustado;
}
